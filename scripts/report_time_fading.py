"""Report time-only fading versus matched fresh unfaded controls."""
...
from test_time_fading import CASES, DOCS, ROOT, schedule, signature
from test_time_fading import TimeObjective as Objective
...
def main():
    ...
    for index in range(14):
        ...
        assert d["variant"] == ("time_annealed" if index % 2 else "unfaded")
        fit = d["fit"]
        trace = fit["trajectory"]
        assert 2000 <= fit["updates"] <= 3000
        assert fit["best_loss"] == min(s["canonical_loss"] for s in trace)
        if d["variant"] == "time_annealed":
            assert trace[0]["horizons"] == [None, None] and trace[-1]["horizons"] == [0.25, None]
        records.append(d)
    ...
    for scope in ("all_seven", "other_five"):
        for state in ("best", "last"):
            for variant in ("unfaded", "time_annealed"):
                group = [
                    r
                    for r in rows
                    if r["state"] == state
    ...
    lines = [
        ...
        "",
        "Only the time horizon shrinks from infinity to 0.25 seconds over 2000 updates. "
        "Both methods share the schedule and enable patience after update 2000. "
        "Frequency accumulation remains completely unfaded. "
        "Fresh starts, fixed amplitudes, no Log-Weighing.",
        "",
    ...
    for case in CASES:
        ...
        cells = [
            ...
            for v in ("unfaded", "time_annealed")
        ]
    ...
    lines += [
        ...
        "the final time horizon. Final patience monitors each fixed terminal objective, while the "
    ...
    ax.semilogy(steps, h[:, 0].astype(float), label="Time horizon (s)")
