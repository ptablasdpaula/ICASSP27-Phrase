"""Registered Adam schedule with separate rotating training and fixed monitoring losses.

Production src/optimization.py is unchanged; only objective/initial injection is added.
"""
...
from icassp27_phrase.losses import canonical_loss_name
...
def fit(
    ...
    *,
    ...
    objective: Callable[[Tensor, int], tuple[Tensor, Tensor]] | None = None,
    ...
    canonical_name = loss_name if objective is not None else canonical_loss_name(loss_name)
    ...
    if objective is None:
        raise ValueError("provide a training/monitor objective")
    ...
    while True:
        ...
        values = objective(audio, update_count)
        value_tensor, monitor_tensor = values
        if value_tensor.ndim != 0:
            raise RuntimeError("a single fit must produce a scalar loss")
        if not all(
            bool(torch.isfinite(value.detach()).all())
            for value in (raw, f0_hz, onset_seconds, audio, value_tensor, monitor_tensor)
        ):
            raise FloatingPointError("non-finite value at an optimisation boundary")
        value = float(monitor_tensor.detach())
        training_value = float(value_tensor.detach())

        if initial_loss is None:
            if value <= 0.0:
                raise FloatingPointError("initial loss must be positive")
            initial_loss = value
            initial_training_loss = training_value
            if initial_training_loss <= 0:
                raise FloatingPointError("initial training loss must be positive")
            best_loss = value
        ...
        conditioned = value_tensor / initial_training_loss
