#!/usr/bin/env python3
"""Show the submitted sine sweep and its four cumulative-power images."""
...
from icassp27_phrase.synth.config import SAMPLE_RATE
from matplotlib.patches import FancyArrowPatch

LANDSCAPE_FIGURE_STYLE = SimpleNamespace(
    arrow_color="white", arrow_half_length=0.026, arrow_mutation_scale=4.2, arrow_linewidth=0.48
)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-stem",
        type=resolve_path,
        default=OUTPUT / "figures/cumulative_surfaces",
    )
    ...
    with torch.no_grad():
        times = (
            torch.arange(
                int(SAMPLE_RATE * DURATION_SECONDS), dtype=torch.float64, device=args.device
            )
            / SAMPLE_RATE
        )
        ...
        for index, (name, time_reverse, frequency_reverse, _, _) in enumerate(DIRECTIONS):
            time_surface = reverse_cumsum(power, 1) if time_reverse else power.cumsum(1)
            surface = (
                reverse_cumsum(time_surface, 0) if frequency_reverse else time_surface.cumsum(0)
            )
    ...
    def display(value):
        return (
            10 * np.log10(np.maximum(value, 10 ** (args.db_floor / 10))) if args.decibels else value
        )

    # STFT coordinates are window centres, rather than artificially shifted onsets.
    time_edges = (
        np.arange(power.shape[1] + 1) * CEL_HOP + CEL_N_FFT / 2 - CEL_HOP / 2
    ) / SAMPLE_RATE
    ...
    common = dict(
        cmap="magma",
        vmin=args.db_floor if args.decibels else 0,
        vmax=0 if args.decibels else 1,
        shading="flat",
        rasterized=True,
    )
    ...
    for axis, surface, (_, time_reverse, frequency_reverse, start, _) in zip(
        ...
        axis.set_title(
            r"$(\mathrm{d}_{\mathrm{t}},\mathrm{d}_{\mathrm{f}})= ("
            + time_arrow
            + ","
            + frequency_arrow
            + ")$",
    ...
    for axis in axes:
        ...
        ticks = [tick for tick in (100, 1000) if args.min_frequency <= tick <= args.max_frequency]
    ...
    figure.supxlabel("Time (s)", x=(axes[0].get_position().x0 + bounds.x1) / 2, y=0.06, fontsize=7)
    ...
    colorbar.set_label(
        "Relative power (dB)" if args.decibels else "Normalised power",
        fontsize=7,
        labelpad=9,
        rotation=270,
    )
    ...
    provenance = {
        ...
        "display": {
            "spectrogram": "power / peak STFT power",
            "surfaces": "S_q/m; accumulation uses linear power",
            "colour_scale": f"10 log10, [{args.db_floor:g},0] dB"
            if args.decibels
            else "linear [0,1]",
            "frequency_axis": (
                f"logarithmic, {args.min_frequency:g}-{args.max_frequency:g} Hz; sums use all bins"
            ),
