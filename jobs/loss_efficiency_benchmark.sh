#!/usr/bin/env bash
#SBATCH --job-name=fixed-efficiency
...
#SBATCH --output=results/loss-efficiency/fixed-%A_%a.log
...
exec "$PWD/.pixi/envs/default/bin/python" scripts/benchmark_loss_efficiency.py \
  ...
  --output "results/loss-efficiency/fixed-${SLURM_ARRAY_JOB_ID:-${SLURM_JOB_ID}}_${SLURM_ARRAY_TASK_ID:-0}.json"
