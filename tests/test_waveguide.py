...
@pytest.mark.parametrize(
    ("method", "orders"),
    (("linear", (1, 1)), ("lagrange", (5, 1)), ("thiran", (3, 1))),
)
...
def test_paper_thiran_transfer_has_expected_physical_lengths() -> None:
    ...
    assert float(transfer.long_delay_samples) == pytest.approx(9.528, abs=0.01)
    assert float(transfer.short_delay_samples) == pytest.approx(2.846, abs=0.01)
