...
from ._fractional import hermitian_rfft_projection
...
def half_raised_cosine(
    ...
    *,
    ...
    sample = torch.arange(sample_count, dtype=torch.float64, device=onset_seconds.device)[
        None, None, :
    ]
...
def _parallel_delay_type() -> type[nn.Module]:
    ...
    with _FLAMO_LOAD_LOCK:
        ...
        if distribution.version != FLAMO_VERSION:
            raise RuntimeError(f"expected FLAMO {FLAMO_VERSION}, found {distribution.version}")
...
class Exciter(nn.Module):
    """Place half-raised-cosine excitation by Fourier phase-ramp delay."""
    ...
    def event_signals(self, onset_seconds: Tensor) -> Tensor:
        ...
        result = torch.fft.irfft(
            hermitian_rfft_projection(
                self._fourier_spectra(onset_seconds), self.config.fourier_fft_length
            ),
            n=self.config.fourier_fft_length,
            dim=-1,
