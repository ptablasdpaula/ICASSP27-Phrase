"""Compatibility implementations retained for archived exploratory scripts."""
...
SOT_MSS_WINDOWS: Final = (2048, 1024, 512, 256, 128, 64)
...
class PaperBoundLoss(BoundLoss):
    """Compatibility wrapper around the frozen shared paper implementation."""

    def __init__(self, target: Tensor, name: str):
        from .losses import PaperObjectives

        self.objective = PaperObjectives(target, (name,))

    def distances(self, candidate: Tensor) -> Tensor:
        rows, squeezed = _batch(candidate)
        value = self.objective(rows)
...
class SmoothMSSDistance(BoundLoss):
    ...
    def __init__(self, target: Tensor, *, sample_rate: int = SAMPLE_RATE):
        self.target = target.detach()
    ...
    def distances(self, candidate: Tensor) -> Tensor:
        ...
        for n_fft, hop, window, target in zip(
            ...
            magnitude = _stft(
                ...
                center=True, pad_mode="constant").abs()
            value = value + (torch.log1p(magnitude) - target).square().mean(
                dim=(-2, -1))
        return _restore(value / len(SMOOTH_WINDOWS), squeezed)
...
class PublishedSOTCompositeDistance(BoundLoss):
    ...
    def __init__(self, target: Tensor, *, sample_rate: int = SAMPLE_RATE):
        self.target = target.detach()
        self.sample_rate = sample_rate
        self.sot_window = periodic_flattop(2048, target)
        self.target_power = _stft(
            target[None], n_fft=2048, hop=256, window=self.sot_window,
            center=True, pad_mode="constant").abs().square().detach()
        self.positions = torch.linspace(
            0.0, 1.0, 1025, dtype=target.dtype, device=target.device)
    ...
    def distances(self, candidate: Tensor) -> Tensor:
        rows, squeezed = _batch(candidate)
        power = _stft(
            rows, n_fft=2048, hop=256, window=self.sot_window,
            center=True, pad_mode="constant").abs().square()
...
def _linear_projection_geometry(
    ...
    if (frequency_bins < 2 or time_bins < 2 or sample_rate != SAMPLE_RATE
            or n_fft != 1024 or hop != 512):
...
class CumulativeEnergyDistance(BidirectionalCumulativeEnergyDistance):
    ...
    def __init__(self, target: Tensor, *, directions: tuple[str, ...] = CEL_DIRECTIONS,
                 log_weighing: bool = False, sample_rate: int = SAMPLE_RATE):
...
class LossRegistry:
    ...
    def build(self, name: str, target: Tensor) -> BoundLoss:
        ...
        paper_name = {
            ...
            "sot_published_composite": "sot_published_composite",
            "linear_jtfot": "linear_jtfot",
            "log_jtfot": "log_jtfot",
            "bidirectional_cumulative_energy": "cel",
            "log_quadrature_bicul": "log_cel",
        }.get(name)
        if paper_name is not None:
            return PaperBoundLoss(target, paper_name)
        if name == "waveform_l1":
